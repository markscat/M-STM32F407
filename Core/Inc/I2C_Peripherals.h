...
#define EEPROM_with_AT24C02
#define RTC_DS3231_Temp
...
/**
 * 這邊必須要解釋一下：
 * 因為之前只有用到DS1307這個RTC，所以沒有考慮到『其他類型』的RTC
 * 才會決定把所有的參數以及函數的命名全部模糊化,
 * 不然時間久了，會搞不清誰是誰。
 * 目前的案例，是DS1307和DS3231，因為是同系例的產品，所以絕大多數的暫存器參數都可以共用；
 * 只要不設定鬧鐘，只有溫度的暫存器需要新增,其餘都可以共用
 *
 *
 * */

//<DS3231專屬>
#ifdef RTC_DS3231_Temp	         //如果有溫度的話,可以改變這個暫存器
#define RTC_TEMP_MSB_REG  0x11	 // MSB (0x11) 包含整數部分和小數點前兩位
#define RTC_TEMP_LSB_REG  0x12	 // LSB (0x12) 包含小數點後兩位 (0.25°C 的解析度)
#endif
//</DS3231專屬>

#define RTC_I2C_ADDR    	0x68     // RTC 的 I2C 地址
#define RTC_TIME_REG    	0x00     // 时间寄存器起始地址
#define RTC_CTRL_REG    	0x07     // 控制寄存器地址


#define RTC_REG_SECOND 		0x00 	// 秒（BCD格式，bit7 = CH，1 表示停止振盪器）
#define RTC_REG_MINUTE 		0x01	// 分（BCD格式）
#define RTC_REG_HOUR  		0x02	// 時（BCD格式，支援12/24小時模式）
#define RTC_REG_DOW    		0x03	// 星期（1=星期日 ~ 7=星期六）
#define RTC_REG_DATE   		0x04	// 日（BCD格式）
#define RTC_REG_MONTH  		0x05	// 月（BCD格式）
#define RTC_REG_YEAR   		0x06	// 年（BCD格式，範圍 00~99）
#define RTC_REG_CONTROL 	0x07	// 控制暫存器（可啟用方波輸出 SQW/OUT）

#define RTC_REG_UTC_HR		0x08	// [自定義] UTC 小時偏移（例如 +8）
#define RTC_REG_UTC_MIN		0x09	// [自定義] UTC 分鐘偏移（例如 +30）
#define RTC_REG_CENT    	0x10	// [自定義] 世紀（例如 20 = 表示年份為 20xx）
#define RTC_REG_RAM   		0x11	// [自定義] RAM 起始位置（RTC有56 bytes SRAM）
#define RTC_TIMEOUT			1000	// I2C timeout 時間（毫秒）
...
typedef struct {
	int8_t Integer;   // 整數部分
    uint8_t Fraction; // 小數部分, 單位 0.25°C (0,1,2,3)
}DS3231_Temp;
...
EEPROM_Status EEPROM_Read(uint16_t addr, uint8_t* data, uint16_t size);
//無CRC版本
EEPROM_Status EEPROM_Write_NoCRC (uint16_t addr, uint8_t* data, uint16_t size);
EEPROM_Status EEPROM_Read_NoCRC (uint16_t addr, uint8_t* data, uint16_t size);
