...
/**
 * EEPROM_Write_NoCRC (uint16_t addr, uint8_t* data, uint16_t size)
 *
 * @brief 写入数据到EEPROM(無CRC)
...
EEPROM_Status EEPROM_Write_NoCRC (uint16_t addr, uint8_t* data, uint16_t size) {
	...
	if(addr_status != EEPROM_OK) {
		printf("EEPROM_Write Error: Invalid address or size.\n");
		return addr_status;
	}
	/*
	 * 步驟 2: 寫入數據 (無 CRC 模式)
	 * 我們只寫入用戶提供的 'data'，長度為 'size'。
	 */
	printf("EEPROM_Write: Writing %u bytes to address 0x%04X...\n", size, addr);
	// 直接將 'data' 緩衝區的內容寫入 EEPROM
	EEPROM_Status status = Internal_Write(addr, data, size);
	// 檢查底層寫入操作是否成功
	if(status != EEPROM_OK) {
		printf("EEPROM_Write Error: Internal_Write failed.\n");
		return status;
	}
	// 更新健康狀態
	  Update_Health(true);
...
EEPROM_Status EEPROM_Read_NoCRC (uint16_t addr, uint8_t* data, uint16_t size) {
	...
	EEPROM_Status addr_status = Validate_Address(addr, size);
	if(addr_status != EEPROM_OK) {
		// 如果地址無效，直接返回錯誤，不做任何操作
		printf("EEPROM_Read Error: Invalid address or size.\n");
		return addr_status;
	  }

	  /*
	   * 步驟 2: 讀取數據 (無 CRC 模式)
	   * 由於我們現在不關心 CRC，所以只需要讀取用戶請求的 'size' 個字節即可。
	   * 我們可以直接將數據讀入用戶提供的 'data' 緩衝區，省去 malloc/free 的開銷和風險。
	   */
	  printf("EEPROM_Read: Reading %u bytes from address 0x%04X...\n", size, addr);

	  // 直接將數據讀取到目標緩衝區 'data' 中
	  EEPROM_Status status = Internal_Read(addr, data, size);

	  // 檢查底層讀取操作是否成功
	  if(status != EEPROM_OK) {
	      printf("EEPROM_Read Error: Internal_Read failed.\n");
	      return status; // 如果底層I2C讀取失敗，直接返回錯誤
	  }

	  // 打印讀取到的原始數據，用於除錯
	#ifdef EEPROM_Read_debug
	  printf("EEPROM_Read: Successfully read data:\n");
