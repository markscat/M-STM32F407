...
*/
  ...
  TCA9555_Init();  // 初始化 TCA9555 為輸出模式
 ...
 //uint16 mask = TCA9555_GPIO_MASK(port, pin)
  uint8_t position = 0;  // 當前點亮的位置（0~15）
  ...
  while (1)
  {
    /* USER CODE END WHILE */
      ...
      HAL_Delay(500);
      if (output_data != 0x00){
    	  TCA9555_WriteOutput(output_data>>1);  // 關閉 GPIO0.0
      }else{
    	  TCA9555_WriteOutput(output_data>>1);
      }
      */
